  ...
  group('Performance Integration Tests', () {
    ...
    test('パフォーマンス設定ファイルが正しい形式で存在する', () {
      ...
      expect(configFile.existsSync(), isTrue, 
        reason: 'パフォーマンス設定ファイルが存在しない');
      
      final content = configFile.readAsStringSync();
      expect(content, contains('performance_targets'), 
        reason: '設定ファイルにperformance_targetsセクションがない');
      expect(content, contains('fps'), 
        reason: '設定ファイルにFPS設定がない');
      expect(content, contains('memory'), 
        reason: '設定ファイルにメモリ設定がない');
      expect(content, contains('build_time'), 
        reason: '設定ファイルにビルド時間設定がない');
    });
    ...
    test('GitHub Actions設定ファイルが存在する', () {
      ...
      expect(content, contains('Performance Monitoring'), 
        reason: 'ワークフロー名が正しくない');
      expect(content, contains('performance-test'), 
        reason: 'パフォーマンステストジョブが定義されていない');
      expect(content, contains('schedule'), 
        reason: 'スケジュール実行が設定されていない');
    });
        ...
        try {
          ...
          if (result.exitCode != 0) {
            ...
            if (Platform.environment['CI'] == 'true') {
              print('警告: $command コマンドが見つかりません（CI環境）');
            } else {
              fail('必須コマンド $command が見つかりません。インストールしてください。');
            }
          }
        } catch (e) {
          print('コマンド $command の確認でエラー: $e');
        }
      }
    });
    ...
    test('パフォーマンスモニタークラスのインポート確認', () {
      ...
      final monitorFile = File('lib/framework/performance/performance_monitor.dart');
      expect(monitorFile.existsSync(), isTrue, 
        reason: 'PerformanceMonitorクラスファイルが存在しない');
      
      final content = monitorFile.readAsStringSync();
      expect(content, contains('class PerformanceMonitor'), 
        reason: 'PerformanceMonitorクラスが定義されていない');
      expect(content, contains('class PerformanceReport'), 
