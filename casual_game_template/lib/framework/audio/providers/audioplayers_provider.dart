...
class AudioPlayersProvider implements AudioProvider {
  ...
  Future<void> playBgm(String assetId, {bool loop = true}) async {
    ...
    try {
      ...
      if (_config?.debugMode == true) {
        debugPrint('BGM playing: $assetId (loop: $loop, volume: ${_bgmVolume * _masterVolume})');
      }
    } catch (e) {
  ...
  Future<void> setSfxVolume(double volume) async {
    ...
    if (_config?.debugMode == true) {
      debugPrint('SFX volume set: $volume (effective: ${_sfxVolume * _masterVolume})');
    }
  }
  ...
  AudioPlayer? _getAvailableSfxPlayer() {
    ...
    if (_sfxPlayerPool.length < 10) { // 最大10個まで
