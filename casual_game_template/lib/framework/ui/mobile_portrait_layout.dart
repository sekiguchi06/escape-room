...
class MobilePortraitLayout {
  ...
  static PortraitLayoutResult calculateLayout(Vector2 screenSize) {
    ...
    if (kIsWeb) {
      ...
      debugPrint('📱 Web environment detected: forcing iPhone15 size (393x852)');
    } else {
      // 実機環境: デバイス画面サイズを使用
      targetSize = screenSize;
      debugPrint('📱 Native environment: using device size (${screenSize.x}x${screenSize.y})');
    }
    
    debugPrint('📱 Portrait layout calculation: ${targetSize.x}x${targetSize.y}');
    ...
    final adArea = LayoutArea(
      position: Vector2(0, targetSize.y * (menuRatio + gameRatio + inventoryRatio)),
...
class PortraitInventoryLayoutCalculator {
  ...
  Vector2 calculateItemSize(int itemCount) {
    ...
    final itemsPerRow = (itemCount > maxItemsPerRow) ? maxItemsPerRow : itemCount;
  ...
  List<Vector2> calculateItemPositions(int itemCount) {
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      final x = inventoryArea.position.x + spacing + col * (itemSize.x + spacing);
      final y = inventoryArea.position.y + (inventoryArea.size.y * 0.2) + 
...
class InventoryLayoutInfo {
  ...
  /// 表示可能行数
  int get visibleRows => (itemCount / PortraitInventoryLayoutCalculator.maxItemsPerRow).ceil();
...
class PortraitLayoutComponent extends Component {
  ...
  PortraitLayoutResult? calculateLayout(Vector2 screenSize) {
    try {
      ...
      debugPrint('📐 Portrait layout calculated: ${screenSize.x}x${screenSize.y}');
  ...
  bool updateScreenSize(Vector2 newScreenSize) {
    if (_currentScreenSize != newScreenSize) {
      debugPrint('📐 Screen size changed: ${_currentScreenSize.x}x${_currentScreenSize.y} → ${newScreenSize.x}x${newScreenSize.y}');
