...
class ScreenFactory {
  ...
  /// スタート画面の生成
  static PositionComponent _createStartScreen(Vector2 screenSize, ScreenConfig config) {
    ...
    // タイトル
    screen.add(FlameUIBuilder.titleText(
    ...
    if (config.subtitle != null) {
      screen.add(FlameUIBuilder.subtitleText(
    ...
    // START GAME ボタン
    screen.add(FlameUIBuilder.primaryButton(
      text: 'START GAME',
    ...
    if (config.showSettings) {
      screen.add(FlameUIBuilder.settingsButton(
        screenSize,
  ...
  /// メニュー画面の生成
  static PositionComponent _createMenuScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    ...
    for (int i = 0; i < menuItems.length; i++) {
      ...
      screen.add(FlameUIBuilder.primaryButton(
        ...
        screenSize: screenSize,
        customPosition: Vector2(screenSize.x / 2 - 100, screenSize.y / 2 + yPos),
      ));
    }
  ...
  /// プレイ画面の生成
  static PositionComponent _createPlayingScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    
    // 背景（ゲームプレイ用・透明度低め）
    screen.add(_createBackground(
      screenSize,
      config.copyWith(backgroundColor: config.backgroundColor?.withValues(alpha: 0.1)),
    ));
    ...
    if (config.showPauseButton) {
      screen.add(FlameUIBuilder.iconButton(
        text: '⏸',
    ...
    if (config.showProgressBar) {
      screen.add(FlameUIBuilder.progressBar(
        screenSize: screenSize,
  ...
  /// 一時停止画面の生成
  static PositionComponent _createPauseScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    
    // 半透明背景（オーバーレイ）
    screen.add(FlameUIBuilder.background(
      screenSize: screenSize,
    ...
    final pausePanel = FlameUIBuilder.panel(
      ...
      children: [
        ...
        FlameUIBuilder.primaryButton(
          ...
          screenSize: Vector2(screenSize.x * 0.8, screenSize.y * 0.6),
          customPosition: Vector2(screenSize.x * 0.4 - 100, screenSize.y * 0.3 - 10),
        ),
        FlameUIBuilder.secondaryButton(
          text: 'Menu',
          onPressed: config.customActions?['menu'] ?? () {},
          screenSize: Vector2(screenSize.x * 0.8, screenSize.y * 0.6),
          customPosition: Vector2(screenSize.x * 0.4 - 75, screenSize.y * 0.3 + 50),
        ),
      ],
    );
  ...
  /// ゲームオーバー画面の生成
  static PositionComponent _createGameOverScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    ...
    // MENU ボタン
    screen.add(FlameUIBuilder.secondaryButton(
      text: 'MENU',
      onPressed: config.customActions?['menu'] ?? () {},
      screenSize: screenSize,
    ));
  ...
  /// 設定画面の生成
  static PositionComponent _createSettingsScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    ...
    // 設定項目リスト（簡易実装）
    final settingsItems = config.settingsItems ?? ['Sound', 'Music', 'Difficulty'];
    for (int i = 0; i < settingsItems.length; i++) {
      final yPos = -50.0 + (i * 50.0);
      screen.add(FlameUIBuilder.secondaryButton(
        text: settingsItems[i],
        onPressed: config.customActions?[settingsItems[i].toLowerCase()] ?? () {},
        screenSize: screenSize,
        customPosition: Vector2(screenSize.x / 2 - 75, screenSize.y / 2 + yPos),
      ));
    }
    
    // 戻るボタン
    screen.add(FlameUIBuilder.backButton(
      screenSize,
  ...
  /// ショップ画面の生成
  static PositionComponent _createShopScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    ...
    // コイン表示（右上）
    screen.add(FlameUIBuilder.scoreText(
      text: config.coinText ?? 'Coins: 100',
      screenSize: screenSize,
    ));
    
    // 商品リスト（グリッド配置・簡易実装）
    final shopItems = config.shopItems ?? ['Power Up', 'Extra Life', 'Coin Pack'];
    for (int i = 0; i < shopItems.length; i++) {
      final xPos = (i % 2) * 200.0 - 100.0;
      final yPos = (i ~/ 2) * 80.0 - 50.0;
      screen.add(FlameUIBuilder.secondaryButton(
        text: shopItems[i],
        onPressed: config.customActions?['buy_${shopItems[i].toLowerCase().replaceAll(' ', '_')}'] ?? () {},
        screenSize: screenSize,
        customSize: Vector2(180, 60),
        customPosition: Vector2(screenSize.x / 2 + xPos, screenSize.y / 2 + yPos),
      ));
    }
  ...
  /// リーダーボード画面の生成
  static PositionComponent _createLeaderboardScreen(Vector2 screenSize, ScreenConfig config) {
    final screen = PositionComponent();
    ...
    // スコアリスト（簡易実装）
    final scores = config.leaderboardData ?? [
