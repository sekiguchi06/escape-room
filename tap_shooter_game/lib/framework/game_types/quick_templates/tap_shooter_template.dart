...
abstract class QuickTapShooterTemplate extends ConfigurableGameBase<TapShooterState, TapShooterConfig> {
  // ゲーム要素
  ...
  Future<void> onLoad() async {
    ...
    // 初期状態設定
    managers.stateProvider.changeState(TapShooterState.menu);
  ...
  Future<void> setupGame() async {
    ...
    // タイマー設定
    managers.timerManager.addTimer('gameTimer', TimerConfiguration(
  ...
  @override
  void startGame() {
    managers.stateProvider.changeState(TapShooterState.playing);
    _gameActive = true;
    _score = 0;
    _enemiesDestroyed = 0;
    
    // タイマー開始
    managers.timerManager.getTimer('gameTimer')?.start();
  ...
  void _startEnemySpawning() {
    managers.timerManager.addTimer('enemySpawner', TimerConfiguration(
      duration: const Duration(seconds: 2),
      type: TimerType.interval,
      resetOnComplete: true,
      onComplete: () => _spawnEnemy(),
    ));
    managers.timerManager.getTimer('enemySpawner')?.start();
  }
  ...
  void _onEnemyTapped(EnemyComponent enemy) {
    ...
    // 効果音再生
    managers.audioManager.playSfx('enemy_destroyed');
  ...
  /// ゲーム終了
  void _endGame() {
    managers.stateProvider.changeState(TapShooterState.gameOver);
    _gameActive = false;
  ...
  void pauseGame() {
    if (_gameActive) {
      ...
      managers.stateProvider.changeState(TapShooterState.paused);
      _gameActive = false;
    }
  }
  ...
  @override
  void resumeGame() {
    if (managers.stateProvider.currentState == TapShooterState.paused) {
      resumeEngine();
