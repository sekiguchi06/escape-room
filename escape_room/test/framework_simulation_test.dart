...
void main() {
  ...
  group('フレームワークシミュレーションテスト', () {
    test('ゲーム完全サイクル シミュレーション', () {
      ...
      debugPrint('📍 初期状態: ${stateProvider.currentState.toString()}');
      ...
      debugPrint(
        '📍 プレイ中状態: 残り時間 ${playingState.timeRemaining}秒, セッション ${playingState.sessionNumber}',
      );
      ...
      for (final time in timeSteps) {
        ...
        final currentState = stateProvider
            .getStateAs<SimpleGamePlayingState>()!;
      ...
      final newPlayingState = stateProvider
          .getStateAs<SimpleGamePlayingState>()!;
      ...
      debugPrint(
        '  - セッション平均遷移数: ${stats.averageStateTransitionsPerSession.toStringAsFixed(2)}',
      );
    ...
    test('複数プリセット設定テスト', () {
      ...
      for (final presetName in presets) {
        ...
        final configuration = SimpleGameConfigPresets.getConfigurationPreset(
          presetName,
        );
    ...
    test('設定駆動ゲーム動作テスト', () {
      ...
      const customConfig = SimpleGameConfig(
        ...
        fontSizes: {'start': 18.0, 'playing': 20.0, 'gameOver': 16.0},
        fontWeights: {
      ...
      stateProvider.startGame(
        customConfig.gameDuration.inMilliseconds / 1000.0,
      );
      ...
      for (final time in timeSteps) {
        ...
        if (time > 0) {
          ...
          final dynamicText = customConfig.getStateText(
            'playing',
            timeRemaining: time,
          );
          final dynamicColor = customConfig.getDynamicColor(
            'playing',
            timeRemaining: time,
          );
    ...
    test('エラーハンドリング・エッジケーステスト', () {
      ...
      stateProvider.resetToState(SimpleGameStateFactory.createStartState());
      final invalidRestartResult = stateProvider.restart(
        5.0,
      ); // start状態からrestartは無効
