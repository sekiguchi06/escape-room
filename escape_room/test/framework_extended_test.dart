  ...
  group('拡張フレームワーク基盤テスト', () {
    group('音響システム - プロバイダーパターン', () {
      test('SilentAudioProvider - 基本動作', () async {
        ...
        final config = const DefaultAudioConfiguration(
          bgmAssets: {'menu': 'menu_bgm.mp3', 'game': 'game_bgm.mp3'},
          sfxAssets: {'tap': 'tap.wav', 'success': 'success.wav'},
          bgmVolume: 0.7,
          sfxVolume: 0.8,
          debugMode: true,
        );
        ...
        final manager = AudioManager(provider: provider, configuration: config);
        ...
        debugPrint(
          '  ✅ デバッグ情報: BGM有効=${debugInfo['bgm_enabled']}, SFX有効=${debugInfo['sfx_enabled']}',
        );
    ...
    group('データ永続化システム - プロバイダーパターン', () {
      test('LocalStorageProvider - データ操作', () async {
        ...
        final manager = DataManager(provider: provider, configuration: config);
        ...
        debugPrint(
          '  ✅ ゲーム進行状況: レベル${loadedProgress['current_level']}, 解放${loadedProgress['unlocked_levels'].length}個',
        );

        // ストレージ情報確認
        final storageInfo = await manager.getStorageInfo();
        debugPrint(
          '  ✅ ストレージ情報: ${storageInfo['total_keys']}キー, ${storageInfo['total_size_kb']}KB',
        );
    ...
    group('収益化システム - 広告統合抽象化', () {
      test('MockAdProvider - 広告制御', () async {
        ...
        manager.addAdEventListener((event) {
          adEvents.add(event);
          debugPrint(
            '  📊 広告イベント: ${event.adType.name} - ${event.result.name}',
          );
        });
        ...
        expect(
          adEvents.any(
            (e) =>
                e.adType == AdType.interstitial && e.result == AdResult.shown,
          ),
          isTrue,
        );
        ...
        expect(
          totalShows is int ? totalShows : int.parse(totalShows.toString()),
          greaterThan(0),
        );
        debugPrint(
          '  ✅ 収益統計: 総表示${revenueStats['total_shows']}回, 推定収益\$${revenueStats['total_revenue']}',
        );
    ...
    group('分析システム - イベント追跡抽象化', () {
      test('ConsoleAnalyticsProvider - イベント追跡', () async {
        ...
        await manager.trackEvent(
          'test_event',
          parameters: {'test_parameter': 'test_value', 'numeric_value': 42},
        );
        debugPrint('  ✅ 基本イベント追跡成功');

        // ゲーム固有イベント追跡
        await manager.trackGameStart(
          gameConfig: {'difficulty': 'normal', 'game_mode': 'classic'},
        );
        ...
        // 広告・課金イベント
        await manager.trackAdShown(adType: 'interstitial', adId: 'test_ad_123');
    ...
    group('統合シナリオ - 全システム連携', () {
      test('マルチシステム連携動作', () async {
        ...
        for (int i = 0; i < 3; i++) {
          // 入力処理
          inputManager.processor.processTapDown(
            Vector2(100 + (i * 10).toDouble(), 100),
          );
          inputManager.processor.processTapUp(
            Vector2(100 + (i * 10).toDouble(), 100),
          );
          ...
          await analyticsManager.trackEvent(
            'player_action',
            parameters: {'action_type': 'tap', 'score': score},
          );
        ...
        if (adResult == AdResult.shown) {
          await analyticsManager.trackAdShown(
            adType: 'interstitial',
            adId: 'end_game_ad',
          );
