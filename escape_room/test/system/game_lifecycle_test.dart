  ...
  group('🔄 システムテスト - ゲームライフサイクル', () {
    ...
    group('完全ゲームサイクル', () {
      test('ゲーム開始 → プレイ → ゲームオーバー → リスタート', () async {
        ...
        for (int i = 0; i < 30; i++) {
          ...
          if (i % 10 == 0) {
            debugPrint(
              '  📊 Frame $i: Timer=${timer.current.inMilliseconds}ms, State=${game.currentState.name}',
            );
          }
        }
        ...
        // タイマーが正常に減少していることを確認
        expect(
          timer.current.inMilliseconds,
          lessThan(initialTime.inMilliseconds),
        );
        ...
        game.stateProvider.changeState(
          SimpleGamePlayingState(sessionNumber: newSessionNumber),
        );
        ...
        debugPrint(
          '  ✅ Phase 5: リスタート完了 - セッション${newPlayingState.sessionNumber}開始',
        );
    ...
    group('システム統合シナリオ', () {
      test('全システム連携ワークフロー', () async {
        ...
        // === 分析システム: セッション開始 ===
        await game.managers.analyticsManager.trackGameStart(
          gameConfig: {
        ...
        final initialHighScore = await game.managers.dataManager
            .loadHighScore();
        ...
        // === 収益化システム: 広告イベント ===
        final adResult = await game.managers.monetizationManager
            .showInterstitial();
