  ...
  group('統合シナリオテスト', () {
    test('統合シナリオ - 複合ゲームシミュレーション', () {
      ...
      final completedState =
          stateProvider.currentState as TestGameCompletedState;
      expect(completedState.finalLevel, equals(3));
      expect(completedState.completionTime, equals(completionTime));
      debugPrint(
        '    🏆 ゲーム完了: 最終レベル${completedState.finalLevel}, 時間${completedState.completionTime.inSeconds}秒',
      );
      ...
      debugPrint(
        '    🧪 A/Bテスト (hard): 時間${hardConfig.maxTime.inSeconds}秒, レベル${hardConfig.maxLevel}, 難易度x${hardConfig.difficultyMultiplier}',
      );
