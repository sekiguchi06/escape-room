...
void main() {
  group('日本語文字化け問題テスト', () {
    test('Unicode明示文字列の正確性テスト', () {
      ...
      final toolUnicode = String.fromCharCodes([
        12489,
        12521,
        12452,
        12496,
      ...
      debugPrint('✅ Unicode生成文字列: $keyUnicode, $toolUnicode, $memoUnicode');
    });

    test('JapaneseMessageSystemメッセージ取得テスト', () {
      // メッセージ取得テスト
      final gameStart = JapaneseMessageSystem.getMessage('game_start');
      final inventoryTitle = JapaneseMessageSystem.getMessage(
        'inventory_title',
      );
      ...
      debugPrint('✅ メッセージ取得結果:');
      debugPrint('  game_start: $gameStart');
      debugPrint('  inventory_title: $inventoryTitle');
      debugPrint('  item_key: $itemKey');
    ...
    test('Flutter TextStyleとFlame TextPaint比較テスト', () {
      ...
      debugPrint('✅ フォント設定比較:');
      debugPrint('  Flutter: ${flutterStyle.fontFamily}');
      debugPrint('  Flame: ${flameTextPaint.style.fontFamily}');
      debugPrint('  Fallback: ${flameTextPaint.style.fontFamilyFallback}');
    });
    ...
    test('文字エンコーディング直接テスト', () {
      ...
      debugPrint('✅ 文字エンコーディング比較:');
      debugPrint('  直接文字列: "$directString" (${directString.codeUnits})');
      debugPrint('  Unicode配列: "$unicodeString" (${unicodeString.codeUnits})');
      debugPrint('  エスケープ: "$escapeString" (${escapeString.codeUnits})');
    ...
    test('Flame TextComponent生成テスト', () {
      ...
      debugPrint('✅ TextComponent生成成功:');
      debugPrint('  text: "${textComponent.text}"');
      debugPrint('  position: ${textComponent.position}');
    });
    ...
    test('WebFont読み込み状態シミュレーション', () {
      ...
      debugPrint('✅ フォールバック設定確認:');
      debugPrint('  Primary: ${textPaint.style.fontFamily}');
      debugPrint('  Fallback: ${textPaint.style.fontFamilyFallback}');
    });

    test('文字列長・バイト数確認テスト', () {
      final testStrings = ['鍵', 'ドライバー', 'メモ', 'インベントリ', 'ゲーム開始'];
      ...
      for (final str in testStrings) {
        ...
        debugPrint('  "$str":');
        debugPrint('    文字数: ${str.length}');
        debugPrint('    CodeUnits: $codeUnits');
        debugPrint('    Runes: $runes');
        debugPrint(
          '    UTF-16: ${codeUnits.map((u) => '0x${u.toRadixString(16)}').join(', ')}',
        );
