...
import 'package:escape_room/framework/providers/provider_factory.dart';
import 'package:escape_room/framework/audio/audio_system.dart';
import 'package:escape_room/framework/audio/providers/flame_audio_provider.dart';
import 'package:escape_room/framework/audio/providers/audioplayers_provider.dart';
...
import 'package:escape_room/framework/monetization/providers/google_ad_provider.dart';
import 'package:escape_room/framework/analytics/analytics_system.dart';
import 'package:escape_room/framework/analytics/providers/firebase_analytics_provider.dart';
...
void main() {
  ...
  group('⚙️ Provider設定テスト', () {
    ...
    test('収益化設定環境別確認', () {
      final devConfig = ProviderFactory.development()
          .createMonetizationConfiguration();
      final testConfig = ProviderFactory.testing()
          .createMonetizationConfiguration();
      final prodConfig = ProviderFactory.production()
          .createMonetizationConfiguration();
  ...
  group('🔧 Flutter公式準拠性確認', () {
    ...
    test('設定クラス準拠確認', () {
      ...
      expect(
        factory.createPersistenceConfiguration(),
        isA<PersistenceConfiguration>(),
      );
      expect(
        factory.createMonetizationConfiguration(),
        isA<MonetizationConfiguration>(),
      );
      expect(
        factory.createAnalyticsConfiguration(),
        isA<AnalyticsConfiguration>(),
      );
    });
