...
import 'package:escape_room/framework/persistence/flutter_official_persistence_system.dart';
  ...
  group('🗃️ Flutter公式準拠永続化システム テスト', () {
    ...
    group('文字列データ操作テスト', () {
      ...
      test('文字列デフォルト値確認', () {
        ...
        final loadedValue = dataManager.loadString(
          key,
    ...
    group('数値データ操作テスト', () {
      ...
      test('負の数値確認', () async {
        ...
        final negativeDouble = await dataManager.saveDouble(
          'negative_double',
          -99.99,
        );
    ...
    group('ブール値データ操作テスト', () {
      ...
      test('ブール値保存・読み込み確認', () async {
        // true値のテスト
        final saveTrueResult = await dataManager.saveBool(
          'test_bool_true',
          true,
        );
        ...
        // false値のテスト
        final saveFalseResult = await dataManager.saveBool(
          'test_bool_false',
          false,
        );
      ...
      test('ブール値デフォルト値確認', () {
        expect(
          dataManager.loadBool('nonexistent_bool', defaultValue: true),
          isTrue,
        );
        expect(
          dataManager.loadBool('nonexistent_bool', defaultValue: false),
          isFalse,
        );
      });
    });
    ...
    group('JSONデータ操作テスト', () {
      ...
      test('ネストしたJSONオブジェクト確認', () async {
        ...
        // ネストした値の確認
        final playerStats =
            loadedValue?['player']?['stats'] as Map<String, dynamic>?;
        expect(playerStats?['level'], equals(10));
        expect(
          playerStats?['achievements'],
          equals(['first_win', 'speed_demon']),
        );
      });
      ...
      test('不正なJSONデータのハンドリング確認', () async {
        ...
        final loadedValue = dataManager.loadJson(
          'invalid_json',
    ...
    group('ゲーム専用メソッドテスト', () {
      ...
      test('統計データ管理確認', () async {
        ...
        expect(
          loadedStats['achievements'],
          equals(['beginner', 'scorer', 'persistent']),
        );
      });
    });
    ...
    group('Flutter公式準拠性確認', () {
      ...
      test('公式推奨データ型確認', () async {
        ...
        expect(
          await dataManager.saveStringList('test_list', ['a', 'b']),
          isTrue,
        );
