...
mixin FrameworkInitializer<TState extends GameState, TConfig> {
  ...
  Future<void> initializeFramework() async {
    // GameManagersにproviderFactoryを設定
    managers.providerFactory = providerFactory;
    ...
    if (debugMode) {
      debugPrint(
        '🔧 Provider bundle created: ${managers.providerBundle.profile.name}',
      );
    }
    ...
    if (debugMode) {
      ...
      debugPrint(
        '🔊 Audio provider: ${managers.providerBundle.audioProvider.runtimeType}',
      );
      debugPrint(
        '🎯 Input provider: ${managers.providerBundle.inputProcessor.runtimeType}',
      );
      debugPrint(
        '💾 Storage provider: ${managers.providerBundle.storageProvider.runtimeType}',
      );
      debugPrint(
        '💰 Ad provider: ${managers.providerBundle.adProvider.runtimeType}',
      );
      debugPrint(
        '📊 Analytics provider: ${managers.providerBundle.analyticsProvider.runtimeType}',
      );
    }
  }
