...
class UILayoutManager {
  ...
  static Vector2 topRight(
    ...
    return Vector2(screenSize.x - componentSize.x - margin, margin);
  }
  ...
  static Vector2 centerLeft(
    ...
    return Vector2(margin, (screenSize.y - componentSize.y) / 2);
  }
  ...
  static Vector2 topCenter(
    ...
    return Vector2((screenSize.x - componentSize.x) / 2, margin);
  }
  ...
  static List<Vector2> grid(
    ...
    final availableWidth =
        parentSize.x - padding.left - padding.right - (spacing * (columns - 1));
    final availableHeight =
        parentSize.y - padding.top - padding.bottom - (spacing * (rows - 1));
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        final x =
            ...
            (cellWidth - childSize.x) / 2;
        final y =
            padding.top +
            row * (cellHeight + spacing) +
            (cellHeight - childSize.y) / 2;
