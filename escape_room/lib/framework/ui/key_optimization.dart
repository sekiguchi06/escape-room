...
class KeyOptimization {
  ...
  static GlobalKey globalKey([String? debugLabel]) =>
      GlobalKey(debugLabel: debugLabel);

  /// リスト要素用の最適化されたKey生成
  static Key listItemKey(String baseId, int index) =>
      ValueKey('${baseId}_$index');

  /// 動的なウィジェット用のユニークKey生成
  static Key uniqueKey(String prefix) =>
      ValueKey('${prefix}_${DateTime.now().millisecondsSinceEpoch}');
}
...
abstract class OptimizedListWidget extends StatelessWidget
    with KeyOptimizedWidget {
...
class OptimizedInventoryDisplay extends StatelessWidget
    with KeyOptimizedWidget {
