...
class TextUIComponent extends UIComponent<String> {
  ...
  void setText(String text, {String? styleId}) {
    ...
    if (isMounted) {
      ...
      _textComponent.textRenderer = JapaneseFontSystem.getTextPaint(
        18,
        Colors.black,
      );
    }
  ...
  void fadeIn({double duration = 0.5}) {
    _textComponent.add(
      OpacityEffect.to(1.0, EffectController(duration: duration)),
    );
  }
  ...
  void fadeOut({double duration = 0.5}) {
    _textComponent.add(
      OpacityEffect.to(0.0, EffectController(duration: duration)),
    );
  }
}
