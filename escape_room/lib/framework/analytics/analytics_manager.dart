...
class AnalyticsManager {
  ...
  Future<void> updateConfiguration(
    AnalyticsConfiguration newConfiguration,
  ) async {
      ...
      if (_configuration.offlineEventsEnabled) {
        ...
        if (_eventQueue.length > _configuration.maxOfflineEvents) {
          _eventQueue.removeRange(
            0,
            _eventQueue.length - _configuration.maxOfflineEvents,
          );
        }
      }
    }
  ...
  Future<bool> startSession() async {
    ...
    if (success && _configuration.autoTrackingEnabled) {
      await trackEvent(
        'session_start',
        parameters: {'session_id': _currentSessionId!},
      );
    }
  ...
  Future<bool> trackPurchase({
    ...
    return await trackEvent(
      'purchase',
      parameters: {'item_id': itemId, 'price': price, 'currency': currency},
  ...
  Map<String, dynamic> getStatistics() {
    return {
      ...
      'total_tracked_events': _eventCounts.values.fold(
        0,
        (sum, count) => sum + count,
      ),
    };
