...
class FirebaseAnalyticsProvider implements AnalyticsProvider {
  ...
  Future<bool> trackEventBatch(List<AnalyticsEvent> events) async {
    ...
    try {
      ...
      if (_config?.debugMode == true) {
        debugPrint('📊 Firebase Analytics batch tracking ${events.length} events...');
      }
      ...
      if (_config?.debugMode == true) {
        debugPrint('📊 Firebase Analytics batch completed: $successCount/${events.length} events');
      }
  ...
  Future<bool> trackError(String error, String? stackTrace) async {
    ...
    try {
      await _analytics!.logEvent(
        ...
        parameters: {
          'error_message': error,
          if (stackTrace != null) 'stack_trace': stackTrace.substring(0, 
  ...
  Future<void> dispose() async {
    if (_config?.debugMode == true) {
      debugPrint('FirebaseAnalyticsProvider disposed (tracked $_eventCount events)');
    }
