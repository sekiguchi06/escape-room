...
class InteractiveInventoryItem extends SimpleInteractiveElement {
  ...
  Future<void> _loadSpriteOverride(String imagePath) async {
    ...
    try {
      ...
      // 既存のスプライトコンポーネントを削除
      children.whereType<SpriteComponent>().forEach(
      ...
      // フォールバック表示
      children.whereType<RectangleComponent>().forEach(
        (rect) => rect.removeFromParent(),
      );
      add(
        RectangleComponent(
          size: size,
  ...
  Future<void> _renderIconText(String iconText) async {
    try {
      ...
      final iconComponent = TextComponent(
        text: iconText,
        textRenderer: TextPaint(style: TextStyle(fontSize: size.y * 0.6)),
        position: size / 2,
      ...
      // 成功時は背景を透明に
      final backgroundComponent = children
  ...
  void onInteractionCompleted() {
    ...
    debugPrint(
      '🎒 Item $id ($itemName) ${_isSelected ? "selected" : "deselected"}',
    );
