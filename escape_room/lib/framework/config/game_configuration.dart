...
/// 設定変更通知を提供するミックスイン
mixin ConfigurationNotifier<TState extends GameState, TConfig> on GameConfiguration<TState, TConfig>, ChangeNotifier {
  @override
  void updateConfig(TConfig newConfig) {
...
class ABTestManager {
  ...
  /// 実験の定義
  void defineExperiment(String experimentId, List<String> variants, Map<String, int> trafficSplit) {
    // 実験定義とユーザー割り当てロジック
    final totalWeight = trafficSplit.values.fold(0, (sum, weight) => sum + weight);
...
class ConfigurationPresets<TConfig> {
  ...
  /// プリセットをJSONから一括読み込み
  void loadPresetsFromJson(Map<String, dynamic> json, TConfig Function(Map<String, dynamic>) fromJson) {
...
class ConfigurationHistory<TConfig> {
  ...
  /// 特定の時間範囲の履歴を取得
  List<ConfigurationHistoryEntry<TConfig>> getHistoryInRange(DateTime start, DateTime end) {
