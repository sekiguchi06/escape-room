...
class LocalScoreProvider implements ScoreProvider {
  ...
  Future<bool> initialize(ScoreConfiguration config) async {
    try {
      ...
      if (_config?.debugMode == true) {
        debugPrint(
          '🎯 LocalScoreProvider initialized (${_highScores.length} high scores loaded)',
        );
      }
      ...
      debugPrint(
        '⚠️ LocalScoreProvider initialization failed, using mock mode: $e',
      );
      _mockMode = true;
      // Initialization complete (tracked through _prefs)
  ...
  Future<bool> submitToLeaderboard(int score, {String? category}) async {
    ...
    if (_config?.debugMode == true) {
      debugPrint(
        '🎯 Leaderboard submission not implemented, using local storage',
      );
    }
  ...
  int calculateBonus(Map<String, dynamic> bonusData) {
    ...
    if (bonusData.containsKey('timeRemaining')) {
      ...
      final timeMultiplier =
          _config?.scoreMultipliers['timeBonus'] as double? ?? 1.5;
