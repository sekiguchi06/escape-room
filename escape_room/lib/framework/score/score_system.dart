...
class ScoreManager extends Component {
  ...
  Future<bool> initialize({
    ...
    try {
      ...
      if (_provider != null) {
        ...
        if (!success) {
          debugPrint('⚠️ ScoreProvider initialization failed, using local mode');
        }
      }
  ...
  void addScore(int points, {String? category}) {
    ...
    if (_config?.debugMode == true) {
      debugPrint('🎯 Score added: +$actualPoints (total: $_currentScore, combo: $_comboCount)');
    }
  ...
  int applyComboBonus(int baseScore) {
    ...
    final comboMultiplier = _config?.scoreMultipliers['combo'] as double? ?? 2.0;
    final bonus = (baseScore * (comboMultiplier * (_comboCount - 1))).round();
    
    if (_config?.debugMode == true) {
      debugPrint('🎯 Combo bonus applied: x${comboMultiplier.toStringAsFixed(1)} (combo: $_comboCount)');
    }
  ...
  Future<bool> saveHighScore({String? playerName, String? category}) async {
    ...
    try {
      ...
      await _provider?.saveHighScore(_currentScore, playerName: playerName, category: category);
  ...
  Future<bool> submitToLeaderboard({String? category}) async {
    ...
    try {
      final success = await _provider?.submitToLeaderboard(_currentScore, category: category) ?? false;
      
      if (_config?.debugMode == true) {
        debugPrint('🎯 Leaderboard submission: ${success ? 'success' : 'failed'}');
      }
