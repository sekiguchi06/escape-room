...
class InteractableGameObject extends BaseGameObject
    with TapCallbacks
    implements InteractableInterface {
  // コンポーネント
  ...
  InteractionResult performInteraction() {
    if (_interactionStrategy == null) {
      return InteractionResult.failure(
        JapaneseMessageSystem.getMessage('interaction_strategy_not_set'),
      );
    }
      ...
      if (result.success) {
        ...
        for (final itemId in result.itemsToAdd) {
          if (game is EscapeRoomGame) {
            ...
            debugPrint(
              '⚠️ Warning: Could not access EscapeRoomGame for inventory',
            );
          }
        }
  ...
  Map<String, dynamic> getState() {
    ...
    try {
      baseState.addAll({
        ...
        'currentSprite': dualSpriteComponent?.hasSprites == true
            ? 'loaded'
            : 'not_loaded',
      });
    } catch (e) {
