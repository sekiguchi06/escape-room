...
class DualSpriteComponent extends Component {
  ...
  Future<void> _loadSprites() async {
    debugPrint('🔍 DualSprite attempting to load: inactive=$inactiveAssetPath, active=$activeAssetPath');
    try {
      _inactiveSprite = await Sprite.load(inactiveAssetPath);
      _activeSprite = await Sprite.load(activeAssetPath);
      debugPrint('✅ DualSprite loaded successfully: $inactiveAssetPath, $activeAssetPath');
    } catch (e) {
      debugPrint('❌ Failed to load sprites: $e');
      debugPrint('❌ Attempted paths: inactive=$inactiveAssetPath, active=$activeAssetPath');
  ...
  void _createInitialSprite() {
    debugPrint('DualSprite: _createInitialSprite called, _inactiveSprite: ${_inactiveSprite != null}');
