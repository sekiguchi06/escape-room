...
abstract class QuickMatch3Template extends ConfigurableGameBase<Match3State, Match3Config> {
  // グリッドシステム
  ...
  Future<void> onLoad() async {
    ...
    // 初期状態設定
    managers.stateProvider.changeState(Match3State.menu);
  ...
  @override
  void startGame() {
    managers.stateProvider.changeState(Match3State.playing);
    _gameActive = true;
    _score = 0;
    _matchesFound = 0;
  ...
  void _onMatchFound(List<GridCell> matches) {
    ...
    // 効果音再生
    managers.audioManager.playSfx('match_found');
  ...
  /// ゲーム終了
  void _endGame() {
    managers.stateProvider.changeState(Match3State.gameOver);
    _gameActive = false;
  ...
  void pauseGame() {
    if (_gameActive) {
      ...
      managers.stateProvider.changeState(Match3State.paused);
      _gameActive = false;
    }
  }
  ...
  @override
  void resumeGame() {
    if (managers.stateProvider.currentState == Match3State.paused) {
      resumeEngine();
